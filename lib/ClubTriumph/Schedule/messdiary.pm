package ClubTriumph::Schedule::Messdiary;
...
#			$diaryentry->insert;
			...
			if ($attachs{$item}) {
				...
				foreach my $att (@atts) {
					...
					my $attachment = $diaryentry->attach_file($c,$fname,$mount.'/public_html/attachments/'.$fname,$type,$fname,$aitem,1);
