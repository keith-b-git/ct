package ClubTriumph::View::Email::Multipart;
...
around 'process' => sub {
    ...
    if (   $c->stash->{$stash_key}->{attachments}
        && ref $c->stash->{$stash_key}->{attachments} eq 'ARRAY'
        && ref $c->stash->{$stash_key}->{attachments}[0] eq 'HASH' )
    {
		foreach my $attachment ( @{ $c->stash->{$stash_key}->{attachments} } ) {
			push @parts, Email::MIME->create(%{$attachment});
		}
	}
