...
# Created by DBIx::Class::Schema::Loader v0.07049 @ 2019-07-03 19:41:57
# DO NOT MODIFY THIS OR ANYTHING ABOVE! md5sum:h9VYFMWLDHMm9cpxrG8LLg
...
sub news {
	...
	unless ($rows) {$rows = 10}
	unless ($page) {$page = 1}
#	my $news = $self ->blog_menus->search_related('item' => {'contenttype' => '12'},{  order_by => {-desc => 'id'}}); #, order_by => {-desc => 'modified'}
...
sub threads {
	...
	if ($self->type eq 'user_profile') {
		...
		return [$self->items_bytype_viewable_by($user,5,$rows,$page,$sticky)];#->search({},{rows => $rows,page => $page,group_by => 'sortby', order_by => {-desc => ['sticky','sortby']}});
	}
}
...
#	return $self->items_bytype_viewable_count($user,$type);
	my $access_level=256;
	if ($user) {
		$access_level = $user->access_level || 128;
	}
	...
	if ($cache) {
			...
			$count += $cache->get_column('c'.$level) || 0;
			$level = $level / 2;
		}
	}
	return $count;
...
#	return $self->items_bytype_local_count($user,$type);
	...
	if ($cache) {
			while ($level >= $access_level) {
			$count += $cache->get_column('l'.$level ) || 0;
			$level = $level / 2;
		}
	}
	return $count;
...
#		{value => '16', label => 'Competitions Officer'},
	...
	foreach my $option (@access_options) {
		if ((($$option{value} >= $access_level) || ($$option{value} < 2))
				&& ($$option{value} <= $max)
				&& ($$option{value} <= $parent)
				 ) {
...
sub user_level_desc { # returns description of user role by level
	...
	switch ($level) {
		...
		case [32] {
			if ($self->local_group) {
				return $self->title.' group members'
			}
			else {
